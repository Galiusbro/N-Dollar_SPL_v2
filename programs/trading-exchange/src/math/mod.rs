...
pub fn calculate_token_ndollar_value(
    ...
    let ndollar_amount = if token_amount > total_supply / LARGE_POOL_PERCENTAGE {
        // Для крупных продаж учитываем слиппедж
        linear_estimate.checked_mul(SLIPPAGE_ADJUSTMENT_LARGE).and_then(|v| v.checked_div(100))
            .ok_or(TradingError::ArithmeticError)?
    } else {
        linear_estimate
    };
...
pub fn calculate_ndollar_token_amount(
    ...
    // Безопасная оценка с учетом слиппеджа
    ...
    let amount = if linear_estimate > 1000 {
        ...
        linear_estimate.checked_mul(SLIPPAGE_ADJUSTMENT_SMALL).and_then(|v| v.checked_div(100))
            .ok_or(TradingError::ArithmeticError)?
    } else {
        linear_estimate
    };
