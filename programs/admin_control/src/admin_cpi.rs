...
pub mod account {
    ...
    /// Определяет структуру для передачи аккаунтов инструкции upgrade_admin_config
    #[derive(Accounts)]
...
pub fn verify_program_authorization(
    ...
    // Проверяем версию
    if admin_config_data.version < AdminConfig::CURRENT_VERSION {
        msg!("Warning: AdminConfig version {} is older than current version {}", 
             admin_config_data.version, AdminConfig::CURRENT_VERSION);
    }
...
pub mod direct_cpi {
    ...
    /// Вызывает инструкцию обновления версии admin_config через CPI
    pub fn upgrade_admin_config(
        program: AccountInfo<'_>,
        accounts: account::UpgradeAdminConfig<'_>,
    ) -> Result<()> {
        // Создаем дискриминатор для инструкции
        let disc = anchor_lang::solana_program::hash::hash("global:upgrade_admin_config".as_bytes());
        let upgrade_admin_config_discriminator = disc.to_bytes()[..8].to_vec();
        ...
        let ix = solana_program::instruction::Instruction {
            program_id: program.key(),
            accounts: ix_accounts,
            data: upgrade_admin_config_discriminator,
        };
        ...
        solana_program::program::invoke(
            ...
            &[
                accounts.authority.to_account_info(),
                accounts.admin_config.to_account_info(),
            ],
        )?;
    ...
    /// Вызывает инструкцию обновления версии admin_config через CPI с PDA подписью
    pub fn upgrade_admin_config_with_pda_signature(
        program: AccountInfo<'_>,
        accounts: account::UpgradeAdminConfig<'_>,
        seeds: &[&[u8]],
    ) -> Result<()> {
